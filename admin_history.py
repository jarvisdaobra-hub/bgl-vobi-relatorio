...
def previous_month_admin_cash():
    ...
    # Restrict the server-side query to the current fiscal year through the month being audited.
    # This avoids the VOBI 10k historical-result ceiling seen when querying all paid titles.
    query_start = date(period_end.year, 1, 1)
    token = vobi_token()
    raw_rows, api_count = fetch_installments(token, query_start, period_end)

    seen = set()
    matches = []
    paid_rows_in_august = 0
    for row in raw_rows:
        row_id = row.get("id")
    ...
    result = {
        ...
        "paid_rows_in_period": paid_rows_in_august,
