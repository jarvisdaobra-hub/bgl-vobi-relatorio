...
def _fetch_paid_status(token, status):
    ...
    while True:
        ...
        offset += len(batch)
        if offset >= 10000:
            raise RuntimeError(f"VOBI paid status {status} reached 10,000-row offset cap")

    return rows, api_count if api_count is not None else len(rows)


def previous_month_mcmv_cash():
    now = datetime.now(TZ)
    today = now.date()
    ...
    for row in raw_rows:
        ...
        key = str(row_id) if row_id is not None else "|".join(
            str(row.get(k) or "")
            for k in ("paidDate", "number", "price", "totalSplitPrice", "refurbishName", "description")
        )
        if key in seen:
            continue
        seen.add(key)
        ...
        if "mcmv" not in _normalized(project):
            continue
    ...
    projects.sort(key=lambda x: (x["net"], x["income"]), reverse=True)
    ...
    print("MCMV_PREVIOUS_MONTH_CASH " + json.dumps(result, ensure_ascii=False, separators=(",", ":")), flush=True)
    return result
